...
export class DialogService {
	...
	defaults = {
		...
		autoHide: true, //是否自自动消失，配合time参数共用
		time: 5000, //自动消失时间，单位毫秒
		...
		effect: 'fade', //动画效果：fade(默认),newspaper,fall,scaled,flip-horizontal,flip-vertical,sign,false(无动画),
  ...
  showDialog(option) {
		this.dialogBox.prototype = {
			...
			create: function(element) {
				...
				if(!title) {
					dialogHTML[0] = '<section class="dialog-box"><div class="dialog-box-container" style="z-index: 200;"><div class="dialog-box-content"></div>';
				} else {
					if(!hasClose) {
						dialogHTML[0] = '<section class="dialog-box"><div class="dialog-box-container" style="z-index: 200;"><div class="dialog-box-title"><h3>' + title + '</h3></div><div class="dialog-box-content"></div>';
					} else {
						dialogHTML[0] = '<section class="dialog-box"><div class="dialog-box-container" style="z-index: 200;"><div class="dialog-box-title"><h3>' + title + '</h3><span class="dialog-box-close">×</span></div><div class="dialog-box-content"></div>';
					}
				}
				...
				if(!hasBtn) {
					...
					if(confirmValue && cancelValue) {
						dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-cancel">' + cancelValue + '</span><span class="dialog-btn-confirm">' + confirmValue + '</span></div></div><iframe style="position: absolute;z-index: 100;top: 0;width: 100%;height:100%;" frameborder="0" allowtransparency="true" scrolling="no"></iframe></section>';
					} else if(cancelValue) {
						dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-cancel">' + cancelValue + '</span></div></div><iframe style="position: absolute;z-index: 100;top: 0;width: 100%;height:100%;" frameborder="0" allowtransparency="true" scrolling="no"></iframe></section>';
					} else if(confirmValue) {
						dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-confirm">' + confirmValue + '</span></div></div><iframe style="position: absolute;z-index: 100;top: 0;width: 100%;height:100%;" frameborder="0" allowtransparency="true" scrolling="no"></iframe></section>';
					} else {
						dialogHTML[1] = '<div class="dialog-btn"><span class="dialog-btn-cancel">取消</span><span class="dialog-btn-confirm">确定</span></div></div><iframe style="position: absolute;z-index: 100;top: 0;width: 100%;height:100%;" frameborder="0" allowtransparency="true" scrolling="no"></iframe></section>';
					}
				}
			...
			render: function() {
				...
				$this.append(dialogHTML[0] + dialogHTML[1]);
				if(typeof($content) === 'object') { //jquery对象
		...
		this.dialogBox.prototype.settings = $.extend({}, this.defaults, option);
		this.dialogBox.prototype.init();
  }
