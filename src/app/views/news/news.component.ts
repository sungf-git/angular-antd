...
export class NewsComponent implements OnInit {
	...
	/*钩子函数:在 Angular 第一次显示数据绑定和设置指令/组件的输入属性之后，初始化指令/组件。*/
	ngOnInit() {
	...
	/*钩子函数:当 Angular 每次销毁指令/组件之前调用并清扫*/
	ngOnDestroy() {
	...
	goEditPage(row) {
		if(row){
			sessionStorage.setItem('editnews', JSON.stringify(row));
		}else {
			sessionStorage.setItem('editnews', null);
		}
		this.router.navigate(['/main/newsedit']);
	}
	
	requestJsonData(index, size, callback) {
		this.loading = true;
		$.ajax({
			url: "../../../assets/data/newsList.json", //json文件位置
			type: "GET", //请求方式为get
